...
public class JOB_Parser {
    ...
    public static void process(ArrayList<String> files, Map<String, ArrayList<String>> scheme, Set<String> columns)
    {
        for(String file: files)
        {
            try {
                ...
                // file = "q20a.txt";
                File f = new File("JOBBenchmark/Query/" + file);
    ...
    {
        query = query.substring(query.indexOf("from ") + "from ".length(), query.indexOf("where")).trim();
    ...
    public static ArrayList<String> get_queries()
    {
        ...
        File folder = new File("JOBBenchmark/Query");
    ...
    public static ArrayList<String> construct_hypergraph(Map<String, ArrayList<String>> table_columns, String filename)
    {
        ...
        try
        {
            FileWriter  writer = new FileWriter("JOBBenchmark/Hypergraph/" + filename + ".txt");
                                                                    ...
                                                                    Map<String, node> column_node_mapping, Map<String, ArrayList<String>> aliases)
    {
        ...
        for(String table: aliases.keySet())
        {
            ArrayList<String> list = scheme.get(table);
            for(String alias: aliases.get(table))
            {
                ArrayList<String> copy = new ArrayList<>();
                for(String column: list)
                {
                    String temp = alias + "." + column;
                    if(column_node_mapping.containsKey(temp) && column_node_mapping.get(temp).new_name != null)
                    {
                        copy.add(column_node_mapping.get(temp).new_name);
                    }
                    else
                    {
                        copy.add(alias + "." + column);
                    }
                }
                new_scheme.put(alias, copy);
            }
        }

        // for(String s: new_scheme.keySet())
        // {
        //     System.out.println(s + ": " + new_scheme.get(s).toString());
        // }
    ...
    public static Map<String, ArrayList<String>> read_scheme()
    {
        ...
        try {
            File file = new File("JOBBenchmark/Query/scheme.txt");
